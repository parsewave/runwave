...
function drawMarkGridOnScreenshot(file, config = {}) {
  ...
  const lineColor = [255, 36, 36, 0.16];
  const labelBg = [0, 0, 0, 0.24];
  const labelText = [255, 255, 255, 0.78];
  const borderColor = [255, 255, 255, 0.42];
  const maxLabel = String(rows * cols - 1);
  const minCell = Math.min(cellWidth, cellHeight);
  const scale = minCell < textWidth(maxLabel, 2) + 8 ? 1 : 2;
  const padding = scale === 1 ? 2 : 3;

  for (let col = 0; col <= cols; col += 1) {
    drawVerticalLine(png, Math.min(png.width - 1, col * cellWidth), lineColor);
  }
  for (let row = 0; row <= rows; row += 1) {
    drawHorizontalLine(png, Math.min(png.height - 1, row * cellHeight), lineColor);
  }
  ...
  for (let row = 0; row < rows; row += 1) {
    for (let col = 0; col < cols; col += 1) {
      ...
      const x = Math.round(col * cellWidth + (cellWidth - metrics.width) / 2);
      const y = Math.round(row * cellHeight + (cellHeight - metrics.height) / 2);
      fillRect(png, x, y, metrics.width, metrics.height, labelBg);
      drawText(png, text, x + padding, y + padding, labelText, scale);
    }
  }

  drawRect(png, 1, 1, png.width - 2, png.height - 2, borderColor, 1);
  fs.writeFileSync(file, PNG.sync.write(png));
}
