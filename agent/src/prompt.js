...
function buildPlaytesterPrompt({ job, elapsedMs, maxMs, viewport, state, history }) {
  ...
  return [
    ...
    '- If the same action fails about 3-5 times with no visible progress, keep playing but try a different target, control mode, route, or strategy instead of forcing the same action again.',
