...
function buildPlaytesterPrompt({ job, elapsedMs, maxMs, viewport, state, history }) {
  ...
  return [
    ...
    '- If a target, swap, click, key, or drag has failed several times with no visible progress, keep playing but try a different target, control mode, route, or strategy instead of forcing the same action again.',
